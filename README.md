...
| `server/middleware/` | The built-server half of PWA chrome; `scripts/grok-pwa-plugin.mjs` is its dev/preview counterpart. |
| `migrations/` | Schema, applied to PGLite automatically on startup (`npm run db:migrate` applies it to a real `DATABASE_URL`). `migrations/auth/` is the Better Auth schema; do not edit it by hand. |
...
Nothing needs `DATABASE_URL` or auth credentials — PGLite and the baked
shared preview auth client cover the app.
